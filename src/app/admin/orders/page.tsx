  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-silver-900">الطلبات 🛍️</h1>
      ...
      {orders.length === 0 ? (
        <div className="bg-white rounded-2xl border border-silver-100 shadow-card p-16 text-center">
          <p className="text-silver-400 text-lg font-bold">لا توجد طلبات بعد</p>
          <p className="text-silver-300 text-sm mt-1">ستظهر الطلبات هنا بعد استلامها عبر واتساب</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-silver-50 border-b border-silver-100">
                <tr>
                  <th className="px-4 py-3 text-right font-black text-silver-600">رقم الطلب</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">العميل</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">الهاتف</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">المنتجات</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">الإجمالي</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">الحالة</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">التاريخ</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-silver-50/50 transition-colors">
                      <td className="px-4 py-3">
                        <span className="font-mono text-xs bg-silver-100 text-silver-600 px-2 py-1 rounded-lg">
                          {order.orderNumber.slice(-8).toUpperCase()}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <p className="font-bold text-silver-800">{order.customerName}</p>
                        <p className="text-silver-400 text-xs truncate max-w-[140px]">{order.address}</p>
                      </td>
                      <td className="px-4 py-3">
