  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-silver-900">إدارة المنتجات 📦</h1>
      ...
      {/* Table */}
      {products.length === 0 ? (
        <div className="bg-white rounded-2xl border border-silver-100 shadow-card p-16 text-center">
          <p className="text-silver-400 text-lg font-bold">لا توجد منتجات بعد</p>
        ...
        <div className="bg-white rounded-2xl shadow-card border border-silver-100 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-silver-50 border-b border-silver-100">
                <tr>
                  <th className="px-4 py-3 text-right font-black text-silver-600">المنتج</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">الفئة</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">السعر</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">المخزون</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">الحالة</th>
                  <th className="px-4 py-3 text-right font-black text-silver-600">إجراءات</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-silver-50">
                {products.map((product) => (
                  <tr key={product.id} className="hover:bg-silver-50/50 transition-colors">
                    {/* Product info */}
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="relative w-12 h-12 rounded-xl overflow-hidden bg-silver-100 flex-shrink-0 border border-silver-200">
                          {product.images?.[0] ? (
                            <Image
                            ...
                            <div className="w-full h-full flex items-center justify-center text-silver-300 text-xl">📦</div>
                          )}
                        </div>
                        <div className="min-w-0">
                          <p className="font-bold text-silver-800 truncate max-w-[180px]">{product.nameAr}</p>
                          {product.brand && <p className="text-silver-400 text-xs">{product.brand}</p>}
                        </div>
                      </div>
                    </td>
